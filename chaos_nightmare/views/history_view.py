...
class HistoryView(View):
    ...
    def __init__(self, width: int, height: int, color: tuple[int, int, int] = (0,0,0),max_size:int=5) -> None:
        if max_size<=0:
    ...
    def add_frame(self,frame: Image.Image = None):
        ...
        resized_images = [img.resize((image_width,image_height), resample=Image.Resampling.LANCZOS) for img in self.historical]
        ...
        while len(resized_images):
            for i in range(num_images_per_row):
                if len(resized_images)>0:
                    u_0 = i*image_width
                    v_0 = j*image_height
                    self.image.paste(resized_images.pop(),(u_0,v_0,u_0+image_width,v_0+image_height))
