...
class PromptView(View):
    ...
    def _calc_font_size(self,text:str)->int:
        ...
        return int(min([self.font.size,sqrt(float(self.image.width-10)*float(self.image.height-40)/float(len(text))) ]))

    def __init__(self, width: int, height: int, color: tuple = (0, 0, 0), default_font_size:int = 20) -> None:
        
        super().__init__(width, height, "Current Image Prompt",color)
        d1 = ImageDraw.Draw(self.image)
        self.font = ImageFont.truetype("fonts./PressStart2P-Regular.ttf", default_font_size)
    ...
    def update_prompt(self, prompt):
        ...
        logger.info(f"Font calculated to be {font_size} and line width will be set to {line_width}")
