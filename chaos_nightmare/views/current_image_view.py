...
class CurrentImageView(View):
    def __init__(
        self, width: int, height: int, color: tuple[int, int, int] = (0, 0, 0)
    ) -> None:
    ...
    def update_stats(
        self,
        time_till_image_gen: timedelta,
        iteration_counter: int,
        iteration_thresh: int,
        ...
        d1.text(
            (int(w / 2) - int(offset_x / 2), 10),
            col_text,
            fill=self._get_timer_color(time_till_image_gen),
            font=self.font,
        )
        ...
        if generating_image:
            ...
            offset_x = self.font.getlength(gen_text)
            d1.text(
                (int(w / 2) - int(offset_x / 2), int(h / 2)),
                gen_text,
                fill=(255, 255, 255),
                font=self.font,
            )
