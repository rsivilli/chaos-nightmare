...
class View(object):
    ...
    def __init__(
        self, width: int, height: int, title: str = "", color: tuple = (0, 0, 0)
    ) -> None:
    ...
    def draw_border(self):
        ...
        d1.rectangle(
            (1, 1, self.image.width - 1, self.image.height - 1), outline="white"
        )
    ...
    def _add_title(
        ...
        title: str = None,
        vertical_buffer: int = 10,
        img_draw: ImageDraw.ImageDraw = None,
    ):
        if img_draw is None:
    ...
    def draw_bottom(self):
        d1 = self.get_draw_image()
        d1.line(
            [(0, self.image.height - 1), (self.image.width, self.image.height - 1)],
            fill="white",
        )
    ...
    def draw_right(self):
        d1 = self.get_draw_image()
        d1.line(
            [(self.image.width - 1, 0), (self.image.width - 1, self.image.height)],
            fill="white",
        )
