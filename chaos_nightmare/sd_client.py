...
class SDClient(object):
    ...
    async def get_progress(
        self, uri: str = "/sdapi/v1/progress", stream_progress: bool = False
    ) -> SDStatusResponse:
        if self.session is None:
            self.session = ClientSession(self.base_url, trust_env=True)
        async with timeout(10):
            async with self.session.get(
                uri, params={"skip_current_image": str(stream_progress).lower()}
            ) as response:
                data = await response.json()
...
if __name__ == "__main__":
    ...
    try:
        ...
        response = loop.run_until_complete(
            client.img2img(prompt="An alien scene", input_image=response.images[0])
        )
